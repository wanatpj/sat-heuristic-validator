...
def compile_formula(
    f: And, x: torch.Tensor, min_deg: int, max_deg: int
) -> torch.Tensor:
    neg_x = 1 - x
    or_tensors = []
    for or_ in f.xs:
        or_input_tensor = torch.cat(
            [
                neg_x[term_.x.i - 1].reshape(1)
                if isinstance(term_, Neg)
                else x[term_.i - 1].reshape(1)
                for term_ in or_.xs
            ]
        )
...
def min_max_sequential_approximation(f: And) -> bool:
    ...
    for i in range(n):
        ...
        with torch.no_grad():
            x += x.grad
    return (
        compile_formula(f, x=(x >= 0.5).float(), min_deg=1 - n, max_deg=1 + n) > 0.5
    ).item()
